...
- 已完成上新批次，支持将多个发布草稿归入一个 SHEIN 批次，按批次查看草稿数、SKC/SKU 范围、阻断、发布中、审核中、通过和失败数量。
...
1. 将已跑通的 SHEIN 发布链路批量化：支持批次级提交、批次级状态同步、失败任务聚合归因和批量重试。
2. 强化发布任务自动化：在现有手动同步审核状态基础上，增加定时轮询、状态变更通知和审核驳回后的修复工作流。
3. 完善图片转换治理：在已有上传/转换基础上，补充平台图片标识复用、转换结果对比、失败重试和素材质量报表。
4. 强化批量发布阻断修复：支持更多字段类型、跨 SKC/SKU 批量应用、字段变更差异预览和修复历史记录。
5. 完善规则中心治理：为尺码、包装、供货价、SKU 毛重、低倍率清单补充导入批次审计、冲突检测和回滚。
6. 抽象 `PlatformAdapter` 并启动 TEMU：沉淀 SHEIN 的类目、属性、图片、尺码、发布、状态同步接口，再实现 TEMU 元数据验证、草稿字段映射和发布任务骨架。
