...
截至 2026-05-12，P0 已经从“接口可调用”推进到“有持久化、有页面、有操作流水、有批量运营入口、有导出明细”的可体验闭环；P1-A/P1-B 已完成第一版后端、库表和运营页面，进入真实 SHEIN 账号回归阶段。
...
11. 平台商品同步已支持按款号同步和按创建/更新时间范围同步；同步列表后默认补拉 SPU 详情，避免只拿到列表 50 条固定页数据。
12. 平台商品列表已展示商品图片、供货价、品牌名称、类目名称、销售站点摘要，并支持品牌、类目、销售站点筛选。
13. 已从 `/open-api/goods/spu-info` 的 `shelfStatusInfoList` 和各 SKC 原始 payload 展开销售站点明细；列表显示“上架 X 站”，详情页和弹窗展示站点、状态、链接、首次/最近上架时间。
14. 平台商品列表和 SPU 详情均支持批量更新供货价；列表页新增表格导入更新供货价模板、上传解析和按 SPU/SKC 聚合提交。
15. 平台商品列表导出已支持多 Sheet：主表保留统计摘要，`销售站点明细` Sheet 独立保存 SPU 与销售站点关联关系。
...
4. 销售站点能力目前基于读接口展开，仅用于查看、筛选和导出，不代表系统具备修改 SHEIN 站点上架关系的能力。
5. P2 页面仍以规划台账为主，尚未进入后端 adapter、库表和任务流实现。

最近验证：

- `npm test -- scripts/lib/spreadsheet_export.test.mjs scripts/lib/shein_platform_products_ui.test.mjs`：140/140 通过，覆盖平台商品销售站点展示/筛选/导出和多 Sheet 导出。
- `npm test -- scripts/lib/shein_platform_products_persistence.test.mjs scripts/lib/shein_platform_products_ui.test.mjs`：139/139 通过，覆盖平台商品持久化、品牌/类目展示名、销售站点解析和页面接线。
...
| 更新成本价/供货价 | `/open-api/goods/update-cost` | `sheinAdapter.updateCost`；`POST /api/shein-platform-products/:spuName/update-cost`；更新本地 SKU 成本价；详情页 SKU 行操作；列表/详情批量更新；列表表格导入；上涨时读取 `shein_cost_change_reason` | 已完成；待真实账号确认原因字段 |
| SPU 商品详情查询 | `/open-api/goods/spu-info` | `sheinAdapter.queryProductDetail`；`POST /:spuName/sync-detail`；`GET /:spuName/detail`；`shein_platform_skc`、`shein_platform_sku`；详情页 `/shein-platform-products/:spuName`；展开 `shelfStatusInfoList` 销售站点明细 | 已完成同步、落库、详情页和销售站点运营视图 |
| 商品列表/平台标识回捞 | `/open-api/openapi-business-backend/product/query` | `sheinAdapter.queryProductList`；`POST /api/shein-platform-products/sync`；`GET /api/shein-platform-products`；`shein_platform_product`；支持时间范围、款号同步、品牌/类目/站点筛选和导出 | 已完成列表同步、分页、搜索、筛选、导出和持久化 |
...
4. 用真实数据确认 `shelfStatusInfoList` 在 SPU 与 SKC 维度的覆盖口径，必要时补充销售站点数据质量提示。
5. 将 `searchProduct` 是否纳入平台商品列表作为补充搜索源做一次评估；若返回字段稳定，再与本地商品表合并。
6. 下架/撤回、状态同步、异常处理继续扩展，其中“撤回”已有基础动作，“下架”需要等 SHEIN 文档中对应接口和业务口径明确。
...
1. 先做 P0/P1 真实 SHEIN 数据回归，不继续堆模拟闭环。目标是拿真实账号验证平台商品列表、按款号同步、按时间范围同步、站点币种、SPU 详情、销售站点明细、可编辑检查、状态同步、更新成本价、表格导入供货价、编辑、拼款、撤回、平台标识、SKU 查重、条码尺码、打印任务、涨价原因和审核聚合，并沉淀成功/失败响应、traceId 和运营提示。
...
1. 在 `平台商品列表` 执行同步，确认分页、搜索、品牌/类目/站点筛选、列表落库、平台状态字段。
2. 在 `站点币种` 执行同步，确认币种、站点启用状态、符号字段。
3. 从列表进入 3-5 个 SPU 详情，确认 `spu-info` 能稳定落库 SKC/SKU、图片、供货价、售价、状态和 `shelfStatusInfoList`。
4. 对 1 个测试商品执行 `检查可编辑`、`同步状态`、`更新成本价`、批量更新供货价和表格导入供货价，记录成功/失败响应。
5. 导出平台商品列表，确认主 Sheet 只保留统计摘要，`销售站点明细` Sheet 能按 SPU-站点一行一条展开。
6. 对 1 个允许编辑的测试商品执行常用字段编辑；对 1 个允许拼款的测试商品执行拼款模板提交。
7. 将失败响应补入前端校验、toast 文案和最近操作详情展示。
