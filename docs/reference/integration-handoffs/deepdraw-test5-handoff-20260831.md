...
- `208426140203 / 6513943`：当前资源回读中主尺码表含 `鞋长: 26`（回读别名 `鞋长 -> 尺码`），多平台尺码 15 行存在；但 2026-09-01 线上 `product_archive_submit_log` 只显示对应草稿 create=`10200`、post-create update=`10499`，且日志表只有 `request_summary_json`、没有原始请求 body。因此不能把它当作已知的开放 API 写入成功请求样本。
- `208426140204 / 6511943`：当前资源回读中多平台尺码 15 行存在；但 2026-09-01 线上 `product_archive_submit_log` 只有 duplicate search 命中该产品，没有原始 create/update 请求证据。因此历史确切请求方法和原始字段标记为未知。
- `204426140121-test3 / 6516628`：省略多平台尺码后成功创建；主表及唯品会/天猫/抖音可成功写入。

因此，不能把主尺码表增加“尺码/鞋长”列认定为本次失败根因；已有历史产品只证明该形状可存在并成功回读，不证明当前公开 `dp.product.create/update` 的可写请求形态。
